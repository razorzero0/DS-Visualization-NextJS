...
class DirectedGraph {
    ...
    * @returns {Object} - An object containing the shortest distance, path, and visited edges with weights.
    */
   shortestDistance(startNode, endNode) {
      ...
      // The distance to the startNode is 0
      distance[startNode] = 0;

      for (let i = 0; i < nodes.length - 1; i++) {
         // Find the node with the shortest distance
         const current = this._minDistanceNode(distance, visited);
         visited[current] = true;

         // Update distances to neighbors
         Object.keys(this._graph[current]).forEach((neighbor) => {
            const weight = this._graph[current][neighbor];
      ...
      // Reconstruct the shortest path and collect visited edges with weights
      const shortestPath = [];
      ...
      while (currentNode !== startNode) {
         const prevNode = previousNode[currentNode];
         shortestPath.unshift(currentNode);

         if (prevNode !== null) {
            const edgeWeight = this._graph[prevNode][currentNode];
            visitedEdges.unshift({ nodeStart: prevNode, nodeEnd: currentNode, weight: edgeWeight });
            visitedNodes.unshift({
               Iteration: visitedNodes.length + 1,
               node: currentNode,
               weight: edgeWeight,
               prevNode: prevNode,
            });
      ...
      // Return an object with distance, path, visited edges with weights, and visited nodes
      return {
         distance: distance[endNode] === Infinity ? -1 : distance[endNode],
         path: shortestPath,
    ...
    * @param {object} distance - The distance values.
    * @param {object} visited - The visited flags.
    * @returns {string} - The node with the minimum distance.
    */
   _minDistanceNode(distance, visited) {
